...
import com.valuepotion.analytics.EventHistory;
import com.valuepotion.analytics.PurchaseSummary;
import com.valuepotion.analytics.SessionLifecycle;
...
public class AttributesAnalytics extends AnalyticsDriver {
	...
	protected void addOptions() {
		...
		optionTool.addOption(defectionThresholdOption());
	}
	...
	private static Option defectionThresholdOption() {
	...
	static class AttributesReducer extends AnalyticsReducer<Text, Text, Text, Text> {
		...
		public void setup(Context context) {
			...
			defection = Integer.parseInt(conf.get("valuepotion.analytics.defection-threshold", "30"));
		...
		private String[] merge(String[] a, String[] s) throws Exception {
			...
			String[] dates = (String[]) Attributes.INSTALLATION_DATES.get(a, dataTool);
			String[] times = (String[]) Attributes.INSTALLATION_TIMES.get(a, dataTool);

			if (times.length > 0) {
				int k = 0;
				Set set = new HashSet();
				for (int i = 0; i < times.length; i++) {
					if (!set.contains(times[i])) {
						set.add(times[i]);
						dates[k] = dates[i];
						times[k] = times[i];
						k++;
				...
				dates = Arrays.copyOf(dates, k);
				times = Arrays.copyOf(times, k);
				
				Attributes.INSTALLATION_DATES.set(a, dates, dataTool);
				Attributes.INSTALLATION_TIMES.set(a, times, dataTool);
			}
